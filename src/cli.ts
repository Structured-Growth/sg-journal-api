...
program
	...
	.action(async () => {
		...
		const zohoIntegrationQueueName: string = container.resolve<string>("journalApiQueueName");
		const { handler } = await require("./lambda-sqs");
		queue.subscribe(zohoIntegrationQueueName, async (message, event: Message) => {
			await handler({
				Records: [
					{
